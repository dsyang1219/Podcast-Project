...
several lookback windows? Produces the survival tradeoff curve used to pick H.
    ...
    python -m step1_frame.budget [--as-of YYYY-MM-DD] [--sample-h H --sample-window M --seed S]
...
Reproducibility:
  ...
  * feeds are read from data/cache/rss (populated by step1_frame.run); nothing is
...
def _load_corpus() -> pd.DataFrame:
    ...
    if not path.exists():
        raise SystemExit("corpus.csv not found — run `python -m step1_frame.run` first.")
