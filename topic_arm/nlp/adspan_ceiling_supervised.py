...
def run_transformer(targets: dict[str, pd.Series], model_name: str,
    ...
    for tname, y_series in targets.items():
        ...
        # Folds are seed-independent, so every seed is scored on the same
        # train/test partition and the spread below is pure run-to-run noise.
        folds = list(gkf.split(shows, groups=shows))
        ...
        for seed in seeds:
            ...
            for fold, (tr, te) in enumerate(folds):
                ...
                # pandas 3.0 drops the grouping column from groupby.apply results;
                # select the columns explicitly so collection_id survives for the
                # per-show pooling below.
                tr_df = sub[sub.collection_id.isin(tr_shows)]
                tr_df = (tr_df.groupby("collection_id", group_keys=False)[list(tr_df.columns)]
                              .apply(lambda g: g.sample(min(len(g), TRAIN_CHUNKS_PER_SHOW),
                                                        random_state=seed)))
                te_df = sub[sub.collection_id.isin(te_shows)]
                te_df = (te_df.groupby("collection_id", group_keys=False)[list(te_df.columns)]
                ...
                for ep in range(epochs):
                    for step, batch in enumerate(dl):
                        ...
                        opt.step()
                        opt.zero_grad()
                    print(f"  seed{seed} fold{fold} ep{ep} "
                          f"last_loss={loss.item():.4f}", flush=True)
            ...
            print(f"  seed{seed} outer R2 = {r2:+.3f} "
                  f"[{lo:+.3f}, {hi:+.3f}]", flush=True)
            per_seed.append({"seed": int(seed), "n": int(ok.sum()),
                             "outer_r2": float(r2), "ci_95": [float(lo), float(hi)],
                             "show_ids": [str(s) for s in shows[ok]],
                             "preds": preds_show[ok].tolist()})

        base_ids = per_seed[0]["show_ids"]
        if any(p["show_ids"] != base_ids for p in per_seed):
            raise RuntimeError(f"{tname}: per-seed show coverage differs; "
                               "cannot aggregate across seeds")
        y_ok = np.array([y_map[s] for s in base_ids], dtype=float)
        P = np.vstack([p["preds"] for p in per_seed])
        seed_r2 = np.array([p["outer_r2"] for p in per_seed], dtype=float)

        # CI on EXPECTED single-run performance: resample shows, and for each
        # resample average the R2 across seeds. Show-level uncertainty and seed
        # noise therefore both land in the interval.
        rng = np.random.default_rng(SEED)
        boot = np.empty(N_BOOT)
        for b in range(N_BOOT):
            bidx = rng.integers(0, len(y_ok), size=len(y_ok))
            boot[b] = np.mean([r2_from_predictions(y_ok[bidx], p[bidx]) for p in P])
        lo, hi = float(np.percentile(boot, 2.5)), float(np.percentile(boot, 97.5))

        ens = P.mean(axis=0)
        ens_r2 = float(r2_from_predictions(y_ok, ens))
        ens_lo, ens_hi = bootstrap_r2_ci(y_ok, ens)
        print(f"  MEAN over {len(per_seed)} seeds: R2 = {float(seed_r2.mean()):+.3f} "
              f"[{lo:+.3f}, {hi:+.3f}]   seed range "
              f"[{float(seed_r2.min()):+.3f}, {float(seed_r2.max()):+.3f}]", flush=True)

        out["results"][tname] = {
            "show_ids": base_ids, "n": len(base_ids),
            "outer_r2": float(seed_r2.mean()), "ci_95": [lo, hi],
            "seeds": [int(s) for s in seeds],
            "per_seed_r2": [float(v) for v in seed_r2],
            "seed_min_r2": float(seed_r2.min()),
            "seed_max_r2": float(seed_r2.max()),
            "seed_sd_r2": float(seed_r2.std(ddof=1)) if len(seed_r2) > 1 else 0.0,
            "seed_ensemble_r2": ens_r2,
            "seed_ensemble_ci_95": [float(ens_lo), float(ens_hi)],
            "sq_err": ((y_ok - ens) ** 2).tolist(),
            "preds": ens.tolist(),
            "per_seed": per_seed,
            "aggregation": (
                "outer_r2 = mean of per-seed R2, i.e. expected performance of ONE "
                "run; ci_95 = show-level bootstrap of that mean. seed_ensemble_* "
                "averages predictions across seeds and is a DIFFERENT, stronger "
                "estimator -- do not report it as the single-run result. sq_err "
                "is the ensemble's, for paired tests that need a stable vector."),
        }
    return out
...
def main() -> None:
    ...
    ap.add_argument("--seeds", default="0,1,2",
                    help="comma-separated seeds for the transformer arm; the "
                         "reported R2 is their mean, with the spread recorded")
    args = ap.parse_args()
    seeds = [int(s) for s in args.seeds.split(",") if s.strip()]
    if not seeds:
        ap.error("--seeds must name at least one seed")
