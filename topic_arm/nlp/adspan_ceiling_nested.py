...
from step1_frame import config as pipeline_config
...
# SERIAL inner CV, measured rather than assumed. The inner fits are tiny (a
# 203x75 ridge is ~1ms), so joblib's per-GridSearchCV process-pool spawn costs
# far more than the work it parallelizes -- and the outer loop pays that cost
# 204 times per model. With n_jobs=4 and a concurrent multithreaded-BLAS R job
# holding 14 of 20 cores, the ridge arm ran >55 min without finishing what took
# 29s standalone. n_jobs=1 removes the pool entirely.
#
# The exception is RandomForest, whose 300 trees DO parallelize usefully within
# a single fit, so it gets its own worker count via RF_N_JOBS.
N_JOBS = 1
RF_N_JOBS = 4
...
def make_rf():
    def f(Xtr, ytr, Xte):
        g = _grid(RandomForestRegressor(n_estimators=300, random_state=SEED,
                                        n_jobs=RF_N_JOBS),
