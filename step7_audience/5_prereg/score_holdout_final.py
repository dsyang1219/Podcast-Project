"""CONFIRMATORY scoring of the out-of-frame transcripts with the pinned corpus pipeline (750-char chunks, remeasure2 features, nw-weighted show rates, z against the 194-show reference). Writes inputs/holdout_features_final.csv and inputs/holdout_show_dirz_FINAL.csv."""
...
try:
    ...
    pass
t0 = time.time()
# Chunk every holdout transcript exactly as the corpus was chunked; keep each chunk's position (index / count) for the format score.
rows = []
for p in sorted(glob.glob("data/transcripts/HOLDOUT_*/*.json")):
    show = p.split("/")[2]  # folder name, e.g. HOLDOUT_rogan, is the show id
...
print(
    ...
    {k.replace("HOLDOUT_", ""): v for k, v in CH.groupby("show_id").episode_id.nunique().items()},
    flush=True,
)
...
F.to_csv(INPUTS / "holdout_features_final.csv", index=False)  # passage-level features of record
...
S.to_csv(INPUTS / "holdout_show_dirz_FINAL.csv", index=False)
# Print the final scores: small values with sign and two decimals, counts as integers.
print("\n=== FINAL holdout show scores (corpus scale) ===")
print(
    S[
        [
            "show_id",
